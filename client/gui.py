...
def run_client():
    try:
        global app_thread
        app_thread = Thread(target=app.start_client).start()
        messagebox.showinfo("Client", "Client is now running...")

    except Exception as error:
        print("Error occurred trying to start the client", str(error))
...
def main():
    ...
    tk.Button(win, text="Run Client", command=run_client, height=2, width=20).pack(pady=10)
